...
Received: from lists.ozlabs.org (lists.ozlabs.org [IPv6:2404:9400:2:0:216:3eff:fee1:b9f1])
	by mail.lfdr.de (Postfix) with ESMTPS id 81A207190A6
	for <lists+linux-erofs@lfdr.de>; Thu,  1 Jun 2023 04:44:19 +0200 (CEST)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4QWr7P1Jylz3dtZ
	for <lists+linux-erofs@lfdr.de>; Thu,  1 Jun 2023 12:44:17 +1000 (AEST)
...
Received: from out30-113.freemail.mail.aliyun.com (out30-113.freemail.mail.aliyun.com [115.124.30.113])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4QWr742VJWz3c3W
	for <linux-erofs@lists.ozlabs.org>; Thu,  1 Jun 2023 12:43:59 +1000 (AEST)
X-Alimail-AntiSpam: AC=PASS;BC=-1|-1;BR=01201311R121e4;CH=green;DM=||false|;DS=||;FP=0|-1|-1|-1|0|-1|-1|-1;HT=ay29a033018045168;MF=jefflexu@linux.alibaba.com;NM=1;PH=DS;RN=5;SR=0;TI=SMTPD_---0VjzsOLE_1685587434;
Received: from localhost(mailfrom:jefflexu@linux.alibaba.com fp:SMTPD_---0VjzsOLE_1685587434)
          by smtp.aliyun-inc.com;
          Thu, 01 Jun 2023 10:43:55 +0800
From: Jingbo Xu <jefflexu@linux.alibaba.com>
To: xiang@kernel.org,
	chao@kernel.org,
	huyue2@coolpad.com,
	linux-erofs@lists.ozlabs.org
Subject: [PATCH v5 6/6] erofs: use separate xattr parsers for listxattr/getxattr
Date: Thu,  1 Jun 2023 10:43:47 +0800
Message-Id: <20230601024347.108469-7-jefflexu@linux.alibaba.com>
...
---
 fs/erofs/xattr.c | 359 +++++++++++++++++++----------------------------
 1 file changed, 142 insertions(+), 217 deletions(-)

diff --git a/fs/erofs/xattr.c b/fs/erofs/xattr.c
index 64180e83a63e..1b65e74cb4a0 100644
--- a/fs/erofs/xattr.c
+++ b/fs/erofs/xattr.c
...
@@ -142,183 +142,6 @@ static int erofs_init_inode_xattrs(struct inode *inode)
 	return ret;
 }
...
@@ -371,20 +194,57 @@ const struct xattr_handler *erofs_xattr_handlers[] = {
 	NULL,
 };
...
+
+	/* 2. handle xattr name */
+	err = erofs_xattr_body(it, entry.e_name_len, false);
+	if (err)
+		goto out;
+
+	/* 3. handle xattr value */
...
+	err = erofs_xattr_body(it, value_sz, true);
+	it->ofs = EROFS_XATTR_ALIGN(it->ofs);
+	return err;
+out:
+	it->ofs = it->next_ofs;
...
@@ -457,10 +363,24 @@ static int erofs_iter_inline_xattr(struct erofs_xattr_iter *it,
...
@@ -472,12 +392,9 @@ static int erofs_iter_shared_xattr(struct erofs_xattr_iter *it,
 {
 	struct erofs_inode *const vi = EROFS_I(inode);
...
@@ -487,7 +404,15 @@ static int erofs_iter_shared_xattr(struct erofs_xattr_iter *it,
 		if (IS_ERR(it->kaddr))
 			return PTR_ERR(it->kaddr);
