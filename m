...
Received: from lists.ozlabs.org (lists.ozlabs.org [IPv6:2404:9400:2:0:216:3eff:fee1:b9f1])
	by mail.lfdr.de (Postfix) with ESMTPS id 888F16CB68A
	for <lists+linux-erofs@lfdr.de>; Tue, 28 Mar 2023 08:07:17 +0200 (CEST)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4Plzjb2lV0z3cjF
	for <lists+linux-erofs@lfdr.de>; Tue, 28 Mar 2023 17:07:15 +1100 (AEDT)
...
Received: from out30-130.freemail.mail.aliyun.com (out30-130.freemail.mail.aliyun.com [115.124.30.130])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4PlzjS13W6z3cLC
	for <linux-erofs@lists.ozlabs.org>; Tue, 28 Mar 2023 17:07:06 +1100 (AEDT)
X-Alimail-AntiSpam: AC=PASS;BC=-1|-1;BR=01201311R101e4;CH=green;DM=||false|;DS=||;FP=0|-1|-1|-1|0|-1|-1|-1;HT=ay29a033018046059;MF=hsiangkao@linux.alibaba.com;NM=1;PH=DS;RN=2;SR=0;TI=SMTPD_---0VerrjjF_1679983614;
Received: from e18g06460.et15sqa.tbsite.net(mailfrom:hsiangkao@linux.alibaba.com fp:SMTPD_---0VerrjjF_1679983614)
          by smtp.aliyun-inc.com;
          Tue, 28 Mar 2023 14:07:00 +0800
From: Gao Xiang <hsiangkao@linux.alibaba.com>
To: linux-erofs@lists.ozlabs.org
Subject: [PATCH v2] erofs-utils: introduce tarerofs
Date: Tue, 28 Mar 2023 14:06:53 +0800
Message-Id: <20230328060653.2107-1-hsiangkao@linux.alibaba.com>
...
It mainly aims at two use cases:
 - Convert a tarball (or later tarballs with a merged view) into
   a full EROFS image [--tar=full];

 - Generate an EROFS manifest image to reuse tar data [--tar=index],
   which also enables EROFS 512-byte blocks.

The second use case is mainly prepared for OCI direct mount without
OCI blob unpacking.

[ Note that `--tar=full` generates lots of temporary files for now
  which can impact performance since the original tar stream(s) may
  be non-seekable. ]

Signed-off-by: Gao Xiang <hsiangkao@linux.alibaba.com>
---
RFC: https://lore.kernel.org/r/20230324101948.18799-1-hsiangkao@linux.alibaba.com/

changes since RFC:
 - fix hardlinks, symlinks;
 - more pax support;
 - various bugfixes.
 ...
 lib/tar.c                 | 692 ++++++++++++++++++++++++++++++++++++++
 mkfs/main.c               | 122 +++++--
 9 files changed, 980 insertions(+), 95 deletions(-)
...
@@ -180,27 +178,12 @@ static int comp_subdir(const void *a, const void *b)
 	return strcmp(da->name, db->name);
 }
 
-int erofs_prepare_dir_file(struct erofs_inode *dir, unsigned int nr_subdirs)
+static int erofs_setup_dir_layout(struct erofs_inode *dir, unsigned int nr_subdirs)
 {
...
-	nr_subdirs += 2;
 	...
 	if (!sorted_d)
 		return -ENOMEM;
@@ -216,7 +199,6 @@ int erofs_prepare_dir_file(struct erofs_inode *dir, unsigned int nr_subdirs)
 	free(sorted_d);
...
@@ -234,6 +216,39 @@ int erofs_prepare_dir_file(struct erofs_inode *dir, unsigned int nr_subdirs)
 	return 0;
 }
...
+int erofs_write_file_from_buffer(struct erofs_inode *inode, char *buf)
 {
 	...
 	int ret;
@@ -424,9 +439,9 @@ static int write_uncompressed_file_from_fd(struct erofs_inode *inode, int fd)
 	return 0;
 }
...
@@ -439,28 +454,21 @@ static int erofs_write_file(struct erofs_inode *inode)
...
@@ -964,7 +972,7 @@ static int erofs_fill_inode(struct erofs_inode *inode, struct stat *st,
 	return 0;
 }
...
@@ -975,6 +983,7 @@ static struct erofs_inode *erofs_new_inode(void)
...
+	init_list_head(&inode->i_hash);
 	...
 	return inode;
@@ -1062,7 +1071,12 @@ static int erofs_mkfs_build_tree(struct erofs_inode *dir, struct list_head *dirs
...
+			close(fd);
 		...
 		if (ret)
 			return ret;
@@ -1285,3 +1299,78 @@ struct erofs_inode *erofs_mkfs_build_special_from_fd(int fd, const char *name)
...
+		++nr_subdirs;
+
+	ret = erofs_setup_dir_layout(dir, nr_subdirs);
+	if (ret)
+		return ret;
+
+	ret = erofs_prepare_inode_buffer(dir);
...
+	      " --tar=[full/index]    generate an image from tarball(s)\n"
