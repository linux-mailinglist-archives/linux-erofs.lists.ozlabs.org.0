Return-Path: <linux-erofs+bounces-1050-lists+linux-erofs=lfdr.de@lists.ozlabs.org>
X-Original-To: lists+linux-erofs@lfdr.de
Delivered-To: lists+linux-erofs@lfdr.de
Received: from lists.ozlabs.org (lists.ozlabs.org [IPv6:2404:9400:2:0:216:3eff:fee1:b9f1])
	by mail.lfdr.de (Postfix) with ESMTPS id E8B21B8A061
	for <lists+linux-erofs@lfdr.de>; Fri, 19 Sep 2025 16:39:26 +0200 (CEST)
Received: from boromir.ozlabs.org (localhost [127.0.0.1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4cSwBN3lDtz3cbg;
	Sat, 20 Sep 2025 00:39:24 +1000 (AEST)
X-Original-To: linux-erofs@lists.ozlabs.org
Authentication-Results: lists.ozlabs.org; arc=none smtp.remote-ip=115.124.28.68
ARC-Seal: i=1; a=rsa-sha256; d=lists.ozlabs.org; s=201707; t=1758292764;
	cv=none; b=
ARC-Message-Signature: i=1; a=rsa-sha256; d=lists.ozlabs.org; s=201707;
	t=1758292764; c=relaxed/relaxed;
	bh=yPrUVXcTHXCXwE8FQr0aX10+dPi+44CYq9k27esbwyk=;
	h=From:To:Cc:Subject:Date:Message-ID:In-Reply-To:References:
	 MIME-Version; b=
...
Received: from out28-68.mail.aliyun.com (out28-68.mail.aliyun.com [115.124.28.68])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4cSwBL4Jp8z3cZH
	for <linux-erofs@lists.ozlabs.org>; Sat, 20 Sep 2025 00:39:20 +1000 (AEST)
Received: from HUDSONZHU-MB0.tencent.com(mailfrom:hudson@cyzhu.com fp:SMTPD_---.eixW6Op_1758292751 cluster:ay29)
          by smtp.aliyun-inc.com;
          Fri, 19 Sep 2025 22:39:12 +0800
From: ChengyuZhu6 <hudson@cyzhu.com>
To: linux-erofs@lists.ozlabs.org
Cc: xiang@kernel.org,
	hsiangkao@linux.alibaba.com,
	Chengyu Zhu <hudsonzhu@tencent.com>
Subject: [PATCH v3] erofs-utils: oci: add support for indexing by layer digest
Date: Fri, 19 Sep 2025 22:39:10 +0800
Message-ID: <20250919143910.88365-1-hudson@cyzhu.com>
...
---
 ...
 lib/remotes/oci.c  |  82 +++++++++++++++++------
 mkfs/main.c        |  75 +++++++++++++--------
 mount/main.c       | 160 +++++++++++++++++++++++++++++++++------------
 4 files changed, 234 insertions(+), 89 deletions(-)
...
index 26aec27..dd51fd1 100644
--- a/lib/remotes/oci.c
+++ b/lib/remotes/oci.c
@@ -898,6 +898,19 @@ static int ocierofs_prepare_auth(struct ocierofs_ctx *ctx,
 	return 0;
 }
...
+
 ...
 {
@@ -925,16 +938,32 @@ static int ocierofs_prepare_layers(struct ocierofs_ctx *ctx,
 		goto out_manifest;
 	}
...
@@ -1054,10 +1083,10 @@ static int ocierofs_init(struct ocierofs_ctx *ctx, const struct ocierofs_config
...
+		ctx->layer_digest = NULL;
 	...
 	if (config->platform)
@@ -1190,6 +1219,7 @@ static void ocierofs_ctx_cleanup(struct ocierofs_ctx *ctx)
 	free(ctx->tag);
 	free(ctx->platform);
 	free(ctx->manifest_digest);
...
@@ -1204,8 +1234,13 @@ int ocierofs_build_trees(struct erofs_importer *importer,
 		return ret;
 	}
...
+		}
 		...
 		i = 0;
@@ -1215,25 +1250,26 @@ int ocierofs_build_trees(struct erofs_importer *importer,
 	while (i < end) {
...
@@ -1246,12 +1282,18 @@ static int ocierofs_download_blob_range(struct ocierofs_ctx *ctx, off_t offset,
...
@@ -1265,7 +1307,7 @@ static int ocierofs_download_blob_range(struct ocierofs_ctx *ctx, off_t offset,
...
index 50e2bdb..d5d1df6 100644
--- a/mkfs/main.c
+++ b/mkfs/main.c
@@ -215,7 +215,7 @@ static void usage(int argc, char **argv)
 #endif
...
+		"   [,layer=Y]          Y=layer digest to extract (omit to extract all layers)\n"
 ...
 #endif
@@ -707,13 +707,14 @@ static int mkfs_parse_s3_cfg(char *cfg_str)
...
+	long idx;
 	...
 	if (!options_str)
 		return 0;
@@ -732,40 +733,59 @@ static int mkfs_parse_oci_options(struct ocierofs_config *oci_cfg, char *options
 			if (!oci_cfg->platform)
 				return -ENOMEM;
 		} else {
-			p = strstr(opt, "layer=");
