...
Received: from lists.ozlabs.org (lists.ozlabs.org [112.213.38.117])
	by mail.lfdr.de (Postfix) with ESMTPS id B81E287BCF2
	for <lists+linux-erofs@lfdr.de>; Thu, 14 Mar 2024 13:39:17 +0100 (CET)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4TwRlR40YXz3dRd
	for <lists+linux-erofs@lfdr.de>; Thu, 14 Mar 2024 23:39:15 +1100 (AEDT)
...
Received: from smtp232.sjtu.edu.cn (smtp232.sjtu.edu.cn [202.120.2.232])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4TwRlN27x7z3bnv
	for <linux-erofs@lists.ozlabs.org>; Thu, 14 Mar 2024 23:39:12 +1100 (AEDT)
Received: from proxy188.sjtu.edu.cn (smtp188.sjtu.edu.cn [202.120.2.188])
	by smtp232.sjtu.edu.cn (Postfix) with ESMTPS id B7EEF1008DC80;
	Thu, 14 Mar 2024 20:38:09 +0800 (CST)
Received: from zhaoyf.ipads-lab.se.sjtu.edu.cn (unknown [202.120.40.82])
	by proxy188.sjtu.edu.cn (Postfix) with ESMTPSA id 15E49380D7D;
	Thu, 14 Mar 2024 20:38:07 +0800 (CST)
From: Yifan Zhao <zhaoyifan@sjtu.edu.cn>
To: linux-erofs@lists.ozlabs.org,
	hsiangkao@linux.alibaba.com
Subject: [PATCH v6 5/5] erofs-utils: mkfs: introduce inner-file multi-threaded compression
Date: Thu, 14 Mar 2024 20:37:54 +0800
Message-ID: <20240314123754.1548878-6-zhaoyifan@sjtu.edu.cn>
...
---
 ...
 lib/compressor.c         |   2 +
 mkfs/main.c              |   9 +
 4 files changed, 468 insertions(+), 97 deletions(-)
...
@@ -436,29 +496,33 @@ static bool z_erofs_fixup_deduped_fragment(struct z_erofs_vle_compress_ctx *ctx,
 	return true;
 }
...
+
+	priv = calloc(1, sizeof(struct erofs_compress_wq_private));
+	if (!priv)
+		return NULL;
+
+	priv->queue = malloc(Z_EROFS_COMPR_QUEUE_SZ);
+	if (!priv->queue)
+		goto err_free_priv;
...
+}
+
+int z_erofs_mt_private_init_compr(struct erofs_sb_info *sbi,
+				  struct erofs_compress_wq_private *priv,
+				  unsigned int alg_id, char *alg_name,
+				  unsigned int comp_level,
+				  unsigned int dict_size)
+{
+	struct erofs_compress_cfg *lc = &priv->ccfg[alg_id];
+	int ret;
+
+	if (likely(lc->enable))
+		return 0;
...
+}
+
+void *z_erofs_mt_private_free(struct erofs_workqueue *wq, void *private)
+{
...
+}
+
+void z_erofs_mt_workfn(struct erofs_work *work)
+{
...
+	int ret = 0;
+
+	ret = z_erofs_mt_private_init_compr(ctx->ictx->inode->sbi, priv,
...
+
+		cur->work.func = z_erofs_mt_workfn;
...
+					    cfg.c_mt_workers,
+					    cfg.c_mt_workers << 2,
+					    z_erofs_mt_private_alloc,
+					    z_erofs_mt_private_free);
