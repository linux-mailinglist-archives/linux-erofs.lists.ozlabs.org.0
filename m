...
Received: from lists.ozlabs.org (lists.ozlabs.org [112.213.38.117])
	by mail.lfdr.de (Postfix) with ESMTPS id 0D9C3862AC6
	for <lists+linux-erofs@lfdr.de>; Sun, 25 Feb 2024 15:29:11 +0100 (CET)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4TjR2X64F1z3cGD
	for <lists+linux-erofs@lfdr.de>; Mon, 26 Feb 2024 01:29:08 +1100 (AEDT)
...
Received: from smtp232.sjtu.edu.cn (smtp232.sjtu.edu.cn [202.120.2.232])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4TjR2V3nNCz30Kd
	for <linux-erofs@lists.ozlabs.org>; Mon, 26 Feb 2024 01:29:06 +1100 (AEDT)
Received: from proxy188.sjtu.edu.cn (smtp188.sjtu.edu.cn [202.120.2.188])
	by smtp232.sjtu.edu.cn (Postfix) with ESMTPS id BC57810089E44;
	Sun, 25 Feb 2024 22:28:10 +0800 (CST)
Received: from zhaoyf.ipads-lab.se.sjtu.edu.cn (unknown [202.120.40.82])
	by proxy188.sjtu.edu.cn (Postfix) with ESMTPSA id 8090037C986;
	Sun, 25 Feb 2024 22:28:06 +0800 (CST)
From: Yifan Zhao <zhaoyifan@sjtu.edu.cn>
To: hsiangkao@linux.alibaba.com
Subject: [PATCH v3 4/4] erofs-utils: mkfs: introduce inner-file multi-threaded compression
Date: Sun, 25 Feb 2024 22:27:59 +0800
Message-ID: <20240225142759.340165-5-zhaoyifan@sjtu.edu.cn>
...
+				fctx->pclustersize = roundup(len, blksz);
 			...
 			e->length = len;
@@ -470,7 +563,7 @@ static int __z_erofs_compress_one(struct z_erofs_vle_compress_ctx *ctx,
...
@@ -599,6 +703,7 @@ static int z_erofs_compress_one(struct z_erofs_vle_compress_ctx *ctx)
 {
 	unsigned int len = ctx->tail - ctx->head;
...
@@ -624,7 +729,7 @@ static int z_erofs_compress_one(struct z_erofs_vle_compress_ctx *ctx)
...
@@ -912,11 +1017,308 @@ void z_erofs_drop_inline_pcluster(struct erofs_inode *inode)
...
+	erofs_blk_t blkaddr, compressed_blocks = 0;
 	...
 	int ret;
 	struct erofs_sb_info *sbi = inode->sbi;
@@ -963,8 +1365,8 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
 		}
 	}
 #endif
...
+	inode->z_algorithmtype[0] = ccfg[0].algorithmtype;
 	...
 	inode->idata_size = 0;
@@ -975,82 +1377,93 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
...
@@ -1062,7 +1475,7 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
...
@@ -1257,8 +1670,32 @@ int z_erofs_compress_init(struct erofs_sb_info *sbi, struct erofs_buffer_head *s
 		return -EINVAL;
 	}
...
@@ -1271,5 +1708,22 @@ int z_erofs_compress_exit(void)
 		if (ret)
 			return ret;
 	}
+
