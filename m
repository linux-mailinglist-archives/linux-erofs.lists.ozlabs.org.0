...
Received: from lists.ozlabs.org (lists.ozlabs.org [112.213.38.117])
	by mail.lfdr.de (Postfix) with ESMTPS id D17CE75A7DB
	for <lists+linux-erofs@lfdr.de>; Thu, 20 Jul 2023 09:31:29 +0200 (CEST)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4R64B75Z11z30jT
	for <lists+linux-erofs@lfdr.de>; Thu, 20 Jul 2023 17:31:27 +1000 (AEST)
...
Received: from out30-99.freemail.mail.aliyun.com (out30-99.freemail.mail.aliyun.com [115.124.30.99])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4R649z6ynXz2ytQ
	for <linux-erofs@lists.ozlabs.org>; Thu, 20 Jul 2023 17:31:18 +1000 (AEST)
X-Alimail-AntiSpam: AC=PASS;BC=-1|-1;BR=01201311R111e4;CH=green;DM=||false|;DS=||;FP=0|-1|-1|-1|0|-1|-1|-1;HT=ay29a033018045192;MF=hsiangkao@linux.alibaba.com;NM=1;PH=DS;RN=3;SR=0;TI=SMTPD_---0Vnp8Tl6_1689838265;
Received: from e18g06460.et15sqa.tbsite.net(mailfrom:hsiangkao@linux.alibaba.com fp:SMTPD_---0Vnp8Tl6_1689838265)
          by smtp.aliyun-inc.com;
          Thu, 20 Jul 2023 15:31:13 +0800
From: Gao Xiang <hsiangkao@linux.alibaba.com>
To: linux-erofs@lists.ozlabs.org
Subject: [PATCH v3 2/2] erofs-utils: lib: avoid global sbi dependencies (take 1)
Date: Thu, 20 Jul 2023 15:31:04 +0800
Message-Id: <20230720073104.116439-1-hsiangkao@linux.alibaba.com>
X-Mailer: git-send-email 2.24.4
In-Reply-To: <20230719073319.27996-3-jefflexu@linux.alibaba.com>
References: <20230719073319.27996-3-jefflexu@linux.alibaba.com>
MIME-Version: 1.0
...
I plan to apply this patch.
 ...
 lib/zmap.c                     |  37 ++++---
 mkfs/main.c                    |  62 +++++------
 33 files changed, 640 insertions(+), 541 deletions(-)

diff --git a/dump/main.c b/dump/main.c
index 7e75719..409c851 100644
--- a/dump/main.c
+++ b/dump/main.c
...
index b060e06..821d98c 100644
--- a/fuse/main.c
+++ b/fuse/main.c
@@ -49,6 +49,7 @@ int erofsfuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
 	};
...
+	dir.sbi = &sbi;
 	...
 	if (ret)
 		return ret;
@@ -84,7 +85,7 @@ static int erofsfuse_open(const char *path, struct fuse_file_info *fi)
...
@@ -95,7 +96,7 @@ static int erofsfuse_getattr(const char *path, struct stat *stbuf)
...
@@ -115,6 +116,7 @@ static int erofsfuse_read(const char *path, char *buffer,
...
+	vi.sbi = &sbi;
 	...
 	if (ret)
 		return ret;
@@ -155,6 +157,7 @@ static int erofsfuse_getxattr(const char *path, const char *name, char *value,
...
+	vi.sbi = &sbi;
 	...
 	if (ret)
 		return ret;
@@ -169,6 +172,7 @@ static int erofsfuse_listxattr(const char *path, char *list, size_t size)
...
+	vi.sbi = &sbi;
 	...
 	if (ret)
 		return ret;
@@ -244,7 +248,7 @@ static int optional_opt_func(void *data, const char *arg, int key,
...
@@ -251,52 +253,57 @@ static int write_uncompressed_extent(struct z_erofs_vle_compress_ctx *ctx,
...
@@ -315,23 +322,25 @@ static int z_erofs_fill_inline_data(struct erofs_inode *inode, void *data,
 	return len;
 }
...
@@ -344,15 +353,16 @@ static bool z_erofs_fixup_deduped_fragment(struct z_erofs_vle_compress_ctx *ctx,
 					   unsigned int len)
 {
 	struct erofs_inode *inode = ctx->inode;
+	struct erofs_sb_info *sbi = inode->sbi;
...
@@ -373,7 +383,8 @@ static int vle_compress_one(struct z_erofs_vle_compress_ctx *ctx)
 {
...
@@ -396,13 +407,13 @@ static int vle_compress_one(struct z_erofs_vle_compress_ctx *ctx)
 			if (may_packing) {
...
@@ -418,7 +429,7 @@ static int vle_compress_one(struct z_erofs_vle_compress_ctx *ctx)
 					  erofs_strerror(ret));
 			}
...
+				round_down(ctx->head, erofs_blksiz(sbi));
 			...
 			memmove(ctx->queue, ctx->queue + qh_aligned,
@@ -592,10 +604,10 @@ static void *write_compacted_indexes(u8 *out,
...
-	bool update_blkaddr;
 	...
 	if (destsize == 4)
@@ -606,7 +618,6 @@ static void *write_compacted_indexes(u8 *out,
...
-	update_blkaddr = erofs_sb_has_big_pcluster();
 	...
 	pos = 0;
 	for (i = 0; i < vcnt; ++i) {
@@ -669,12 +680,14 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
...
@@ -714,7 +727,7 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
...
@@ -724,7 +737,7 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
...
@@ -734,7 +747,7 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
...
@@ -744,7 +757,7 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
 		in = parse_legacy_indexes(cv, 2, in);
 		out = write_compacted_indexes(out, cv, &blkaddr,
 					      4, logical_clusterbits, false,
-					      &dummy_head);
...
@@ -754,7 +767,7 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
...
+					      &dummy_head, big_pcluster);
 	...
 	return 0;
@@ -763,12 +776,13 @@ int z_erofs_convert_to_compacted_format(struct erofs_inode *inode,
...
@@ -783,6 +797,7 @@ static void z_erofs_write_mapheader(struct erofs_inode *inode,
...
@@ -808,12 +823,12 @@ void z_erofs_drop_inline_pcluster(struct erofs_inode *inode)
 		u8 *out;
...
@@ -835,7 +850,8 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
...
@@ -851,7 +867,7 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
...
+	inode->z_logical_clusterbits = sbi->blkszbits;
 	if (!cfg.c_legacy_compress && inode->z_logical_clusterbits <= 14) {
 		if (inode->z_logical_clusterbits <= 12)
 			inode->z_advise |= Z_EROFS_ADVISE_COMPACTED_2B;
@@ -860,7 +876,7 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
 		inode->datalayout = EROFS_INODE_COMPRESSED_FULL;
 	}
...
@@ -897,7 +913,7 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
...
+	ctx.pclustersize = z_erofs_get_max_pclustersize(inode);
 	...
 	ctx.head = ctx.tail = 0;
@@ -950,7 +966,7 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
...
+	    compressed_blocks * erofs_blksiz(sbi) + inode->idata_size +
 		...
 		ret = -ENOSPC;
@@ -969,8 +985,8 @@ int erofs_write_compressed_file(struct erofs_inode *inode, int fd)
 	}
...
@@ -1025,12 +1041,13 @@ static int erofs_get_compress_algorithm_id(const char *name)
 	return -ENOTSUP;
 }
...
@@ -1038,7 +1055,7 @@ int z_erofs_build_compr_cfgs(struct erofs_buffer_head *sb_bh)
...
@@ -1049,12 +1066,12 @@ int z_erofs_build_compr_cfgs(struct erofs_buffer_head *sb_bh)
 			return PTR_ERR(bh);
 		}
 		erofs_mapbh(bh->block);
...
@@ -1071,12 +1088,12 @@ int z_erofs_build_compr_cfgs(struct erofs_buffer_head *sb_bh)
 			return PTR_ERR(bh);
 		}
 		erofs_mapbh(bh->block);
...
@@ -1094,19 +1111,19 @@ int z_erofs_build_compr_cfgs(struct erofs_buffer_head *sb_bh)
 			return PTR_ERR(bh);
 		}
 		erofs_mapbh(bh->block);
...
+		ret = erofs_compressor_init(sbi, &erofs_ccfg[i].handle,
 		...
 		if (ret)
 			return ret;
@@ -1121,9 +1138,9 @@ int z_erofs_compress_init(struct erofs_buffer_head *sb_bh)
 			return ret;
...
+			erofs_sb_set_compr_cfgs(sbi);
 	...
 	/*
@@ -1132,7 +1149,7 @@ int z_erofs_compress_init(struct erofs_buffer_head *sb_bh)
 	 */
...
+		erofs_sb_clear_lz4_0padding(sbi);
 	...
 	if (!cfg.c_compr_alg[0])
 		return 0;
@@ -1143,21 +1160,21 @@ int z_erofs_compress_init(struct erofs_buffer_head *sb_bh)
 	 */
...
index 86e28d9..a172bb5 100644
--- a/lib/data.c
+++ b/lib/data.c
@@ -18,27 +18,29 @@ static int erofs_map_blocks_flatmode(struct erofs_inode *inode,
...
+							erofs_blksiz(sbi)) {
 			erofs_err("inline data cross block boundary @ nid %" PRIu64,
 				  vi->nid);
 			DBG_BUGON(1);
@@ -65,6 +67,7 @@ int erofs_map_blocks(struct erofs_inode *inode,
 		struct erofs_map_blocks *map, int flags)
 {
 	struct erofs_inode *vi = inode;
+	struct erofs_sb_info *sbi = inode->sbi;
 	...
 	u64 chunknr;
@@ -92,36 +95,36 @@ int erofs_map_blocks(struct erofs_inode *inode,
...
@@ -158,9 +161,10 @@ int erofs_map_dev(struct erofs_map_dev *map)
 	return 0;
 }
...
@@ -168,11 +172,11 @@ int erofs_read_one_data(struct erofs_map_blocks *map, char *buffer, u64 offset,
 		.m_deviceid = map->m_deviceid,
 		.m_pa = map->m_pa,
 	};
...
+	ret = dev_read(sbi, mdev.m_deviceid, buffer, mdev.m_pa + offset, len);
 	if (ret < 0)
 		return -EIO;
 	return 0;
@@ -219,7 +223,8 @@ static int erofs_read_raw_data(struct erofs_inode *inode, char *buffer,
 			map.m_la = ptr;
 		}
...
+					  eend - map.m_la);
 		if (ret)
 			return ret;
 		ptr = eend;
@@ -231,12 +236,14 @@ int z_erofs_read_one_data(struct erofs_inode *inode,
 			struct erofs_map_blocks *map, char *raw, char *buffer,
 			erofs_off_t skip, erofs_off_t length, bool trimmed)
 {
+	struct erofs_sb_info *sbi = inode->sbi;
...
@@ -253,23 +260,24 @@ int z_erofs_read_one_data(struct erofs_inode *inode,
...
+		    nameoff >= erofs_blksiz(sbi)) {
 			...
 			return -EFSCORRUPTED;
@@ -204,7 +206,10 @@ static int erofs_get_pathname_iter(struct erofs_dir_context *ctx)
 	}
...
+		};
 		...
 		if (ret) {
@@ -230,10 +235,14 @@ static int erofs_get_pathname_iter(struct erofs_dir_context *ctx)
 	return 0;
 }
...
+			       inode_loc + sizeof(*dic),
 		...
 		if (ret < 0)
 			return -EIO;
@@ -114,8 +119,8 @@ int erofs_read_inode_from_disk(struct erofs_inode *vi)
...
@@ -134,10 +139,10 @@ int erofs_read_inode_from_disk(struct erofs_inode *vi)
...
@@ -185,6 +190,7 @@ struct erofs_dirent *find_target_dirent(erofs_nid_t pnid,
 }
...
@@ -194,7 +200,8 @@ int erofs_namei(struct nameidata *nd, const char *name, unsigned int len)
...
@@ -204,7 +211,7 @@ int erofs_namei(struct nameidata *nd, const char *name, unsigned int len)
 	offset = 0;
...
@@ -214,7 +221,7 @@ int erofs_namei(struct nameidata *nd, const char *name, unsigned int len)
...
+		    nameoff >= erofs_blksiz(sbi)) {
 			erofs_err("invalid de[0].nameoff %u @ nid %llu",
 				  nameoff, nid | 0ULL);
 			return -EFSCORRUPTED;
@@ -236,7 +243,7 @@ int erofs_namei(struct nameidata *nd, const char *name, unsigned int len)
...
+	nd->nid = nd->sbi->root_nid;
 	...
 	while (*name == '/')
 		name++;
@@ -266,7 +273,7 @@ static int link_path_walk(const char *name, struct nameidata *nd)
 int erofs_ilookup(const char *path, struct erofs_inode *vi)
 {
 	int ret;
-	struct nameidata nd;
