...
Received: from lists.ozlabs.org (lists.ozlabs.org [112.213.38.117])
	by mail.lfdr.de (Postfix) with ESMTPS id 61CAD74E6EE
	for <lists+linux-erofs@lfdr.de>; Tue, 11 Jul 2023 08:13:02 +0200 (CEST)
Received: from boromir.ozlabs.org (localhost [IPv6:::1])
	by lists.ozlabs.org (Postfix) with ESMTP id 4R0Vsk6JMZz30hG
	for <lists+linux-erofs@lfdr.de>; Tue, 11 Jul 2023 16:12:58 +1000 (AEST)
...
Received: from out30-130.freemail.mail.aliyun.com (out30-130.freemail.mail.aliyun.com [115.124.30.130])
	...
	by lists.ozlabs.org (Postfix) with ESMTPS id 4R0VsW5gKnz2yLV
	for <linux-erofs@lists.ozlabs.org>; Tue, 11 Jul 2023 16:12:46 +1000 (AEST)
X-Alimail-AntiSpam: AC=PASS;BC=-1|-1;BR=01201311R381e4;CH=green;DM=||false|;DS=||;FP=0|-1|-1|-1|0|-1|-1|-1;HT=ay29a033018045192;MF=jefflexu@linux.alibaba.com;NM=1;PH=DS;RN=4;SR=0;TI=SMTPD_---0Vn7PyzB_1689055961;
Received: from localhost(mailfrom:jefflexu@linux.alibaba.com fp:SMTPD_---0Vn7PyzB_1689055961)
          by smtp.aliyun-inc.com;
          Tue, 11 Jul 2023 14:12:42 +0800
From: Jingbo Xu <jefflexu@linux.alibaba.com>
To: hsiangkao@linux.alibaba.com,
	chao@kernel.org,
	huyue2@coolpad.com,
	linux-erofs@lists.ozlabs.org
Subject: [PATCH 2/2] erofs-utils: introduce tarerofs
Date: Tue, 11 Jul 2023 14:12:40 +0800
Message-Id: <20230711061240.23662-2-jefflexu@linux.alibaba.com>
X-Mailer: git-send-email 2.19.1.6.gb485710b
In-Reply-To: <20230711061240.23662-1-jefflexu@linux.alibaba.com>
...
3. set device number for char and block device files (in
tarerofs_parse_tar()).
---
 configure.ac              |   1 +
 ...
 lib/xattr.c               |  46 ++-
 mkfs/main.c               | 134 +++++--
 12 files changed, 1177 insertions(+), 106 deletions(-)
 create mode 100644 include/erofs/tar.h
 create mode 100644 lib/tar.c

diff --git a/configure.ac b/configure.ac
index 2ade490..d23a68a 100644
--- a/configure.ac
+++ b/configure.ac
@@ -126,6 +126,7 @@ AC_CHECK_HEADERS(m4_flatten([
 	fcntl.h
 	getopt.h
 	inttypes.h
+	linux/aufs_type.h
...
@@ -172,13 +173,17 @@ struct erofs_inode {
 	} u;
...
@@ -822,7 +829,7 @@ static bool erofs_should_use_inode_extended(struct erofs_inode *inode)
 	return false;
 }
...
@@ -964,7 +971,7 @@ static int erofs_fill_inode(struct erofs_inode *inode, struct stat *st,
 	return 0;
 }
...
@@ -974,7 +981,9 @@ static struct erofs_inode *erofs_new_inode(void)
...
+	init_list_head(&inode->i_hash);
 	...
 	return inode;
@@ -1044,6 +1053,10 @@ static int erofs_mkfs_build_tree(struct erofs_inode *dir, struct list_head *dirs
 	struct erofs_dentry *d;
...
+
 	...
 	if (ret < 0)
 		return ret;
@@ -1061,8 +1074,15 @@ static int erofs_mkfs_build_tree(struct erofs_inode *dir, struct list_head *dirs
 			}
...
+			ret = 0;
 		...
 		if (ret)
 			return ret;
@@ -1285,3 +1305,82 @@ struct erofs_inode *erofs_mkfs_build_special_from_fd(int fd, const char *name)
...
+}
+
+static struct erofs_inode *tarerofs_init_default_dir(struct erofs_inode *pwd)
+{
+	struct erofs_inode *inode;
+
...
+				d->inode = pwd;
+			} else {
+				inode = tarerofs_init_default_dir(pwd);
+				if (IS_ERR(inode))
...
@@ -130,6 +135,8 @@ static void usage(void)
 }
...
+static bool tar_mode;
 ...
 {
@@ -482,6 +489,15 @@ static int mkfs_parse_options_cfg(int argc, char *argv[])
 			}
...
+			break;
 		case 1:
 			usage();
 			exit(0);
@@ -513,20 +529,24 @@ static int mkfs_parse_options_cfg(int argc, char *argv[])
 		return -ENOMEM;
...
@@ -745,14 +765,24 @@ int main(int argc, char **argv)
 		return 1;
 	}
...
@@ -803,11 +833,13 @@ int main(int argc, char **argv)
 	}
 	if (cfg.c_dedupe)
...
+		err = erofs_generate_devtable();
 	if (err) {
 		erofs_err("failed to generate device table: %s",
 			  erofs_strerror(err));
@@ -876,25 +911,52 @@ int main(int argc, char **argv)
